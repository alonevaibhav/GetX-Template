...
class TranslationController extends GetxController {
  ...
  // Reactive variables
  final _currentLanguage = Rxn<Language>();
  final _previousLanguage = Rxn<Language>(); // Track previous language
  final _isLoading = false.obs;
  final _isInitialized = false.obs;
  final _networkStatus = true.obs;
  final _cacheStats = <String, dynamic>{}.obs;

  // In-memory translation cache with TTL
  final _memoryCache = <String, _CachedTranslation>{}.obs;
  ...
  // Batch processing
  final _batchQueue = <_BatchRequest>[];
  Timer? _batchTimer;
  ...
  void onClose() {
    _batchTimer?.cancel();
    TranslationService.instance.dispose();
    super.onClose();
  }
  ...
  Future<String> translateText(
    ...
    final translationFuture = _performTranslation(text, targetLang,
        sourceLanguage, timeout ?? const Duration(seconds: 10));
